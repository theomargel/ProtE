...
To enable statistical analysis, samples belonging to the same group must be arranged next to each other in the Excel file. Regarding the parameters `group_names` and `case_number`, the user must provide these parameters inside a vector, in the order they appear inside the Excel matrix:

- `group_names`: names of the different groups.
- `case_number`: a vector with the number of samples inside each group.
...
- The parameter `global_threshold` determines if filtering is based on missing values across all groups (`TRUE`) or separately by groups (`FALSE`).
