...
2.  ```dianno()``` accepts as input the DIA-NN (or the FragPipe - DIANN) output files .pg_matrix.tsv or unique_genes_matrix.tsv 
3.  ```pd_single()``` accepts as input the Proteome Discoverer output excel file that contains all sample protein intensities/abundances in one table
4.  ```pd_multi()``` accepts as input separate Proteome Discoverer protein intensity excel files 

All 4 functions expect the input file(s) to be parsed in the parameter ```excel_file```. 

# How to use functions 1-3
To enable statistical analysis, in the input .tsv or .xlsx file, samples (columns) belonging to the same group must be sorted next to each other. For example, samples from an experiment with a 3-groups categorical variable (control, treatment, compound) could be arranged such that: first columns = Control samples, middle columns = Treatment samples, last columns = Compound samples.
...
maximum_quantum <- (excel_file = "C:\Bioprojects\BreastCancer\Proteomics\MaxQuant\protein_pg.xlsx",
                    ...
                    significance = "pV")
```
Now add a second ```\``` to the file path:
...
The Package ProtE consists of 4 functions:
- `pd_multi`
- `pd_single`
- `maximum_quantum`
- `dianno`
...
The `dianno` function is created to analyze the .pg_matrix and unique_genes_matrix tsv files that come as the output of the DIA-NN software.
...
knitr::include_graphics(system.file("extdata", "sdmeanplot.jpeg", package = "ProtE"))
```
...
knitr::include_graphics(system.file("extdata", "imputation_histogram.jpeg", package = "ProtE"))
```
...
knitr::include_graphics(system.file("extdata", "prabrank.jpg", package = "ProtE"))
```
...
knitr::include_graphics(system.file("extdata", "Boxplot.jpeg", package = "ProtE"))
```
...
knitr::include_graphics(system.file("extdata", "ViolinPlot.jpeg", package = "ProtE"))
```
...
knitr::include_graphics(system.file("extdata", "pcaplotall.jpeg", package = "ProtE"))
```
...
knitr::include_graphics(system.file("extdata", "pcaplotsig.jpeg", package = "ProtE"))
```
...
knitr::include_graphics(system.file("extdata", "heatmap.jpeg", package = "ProtE"))
```
