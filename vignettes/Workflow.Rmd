...
- Log2 transformation
- Quantile normalization (after Log2 transformation)
- Cyclic Loess normalization(after Log2 transformation)
...
Because the output of DIA NN and ProteomeDiscoverer sometimes does not include any description of the proteins found, when using the functions `pd_multi`, `pd_single` and `dianno` the parameter `description` will be available. 
If the parameter `description` is set to `TRUE`, a new column named `description` with metadata will be included in the created Excel files. These metadata will be fetched using the UniProt.ws library, based on the first protein of the protein.Ids column in the .pg_matrix Excel file. This process requires internet access for dianno it will be utilized only when the input is the .pg_matrix.tsv and not unique_genes_matrix.tsv. If ProteomeDiscoverer files already include a Description column no fetching will happen. 
...
- mean of the gene intensity 
- Missing values as zeros (0s)
- kNN
- MissRanger
- Limit of Detection (LOD)
- LOD/2
- Sample from the Gaussian distribution of the LOD
