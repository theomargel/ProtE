...
While for the case 4 the path to the folder(s) that contain the .xlsx
files for each sample, should be parsed in the parameter
`pd_single_dir`.
...
generated to facilitate quality check and interpretation. These include:
...
the statistical tests and GSEA. • Plots, with all the plots saved in
.bmp format.
